                      ...
                      <b-modal
                        ...
                        <b-container fluid>
                          <form v-on:submit="Save">
                            ...
                            Description
                            <b-form-input type="text" v-model="description" class="mb-2" required></b-form-input>
...
module.exports = {
  data : function(){
    return {
      ...
      key: "",
      book: [],
      fields: ["id", "title", "description", "Aksi"],
    }
  },
  ...
  methods: {
    ...
    Save : function(){
      ...
      if(this.action === "insert"){
        axios.post(base_url + "/book", form, conf)
        .then(response => {
          this.$bvToast.hide("loadingToast");
          if(this.search == ""){
            this.getData();
    ...
    Drop : function(id){
      ...
      if(confirm('Apakah anda yakin ingin menghapus data ini?')){
        this.$bvToast.show("loadingToast");
        axios.delete(base_url + "/book/delete/" + id, conf)
        .then(response => {
          if(response.data.status == 1){
            this.getData();
            this.$bvToast.hide("loadingToast");
